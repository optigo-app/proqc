...
const handlewtchange = (e) => {
  ...
  const retwtchange = (e) => {
    ...
    if (value === '' || (!isNaN(parseFloat(value)) && parseFloat(value) <= grosswt)) {
      ...
      setWterror("Issue Limit Exceeted");
      setWillremain('0.000');
    }
  };
...
{selectedRowData?.actualUsed?.toFixed(3)}|{selectedRowData.actualPcs} (Ctw|Pcs)
   </p> ):(
  <p className="text-black font-semibold">
                {selectedRowData?.actualUsed?.toFixed(3)}(Gm)
                  </p>
...
<input
  type="text"
  placeholder="Add Wt." 
  className="p-2 border rounded-lg w-20  focus:outline-none"
  value={returnWeight}
...
<input
  ...
  className="p-2 border rounded-lg  w-32  focus:outline-none"
  value={returnWeight}
  ref={WtRef}
  onChange={retwtchange}
/>

<p className="text-gray-500">Pcs.</p>
<input
  type="text"
...
<div className="h-[1rem] my-4">
                {wterror && <p className="text-red-600 text-sm text-center">
                  {wterror}
                  </p>}
              </div>
...
<div 
  className={`bg-gradient-to-r from-orange-300 to-orange-500 m-3 rounded-md  cursor-pointer flex items-center justify-center  text-white px-3 py-3 mt-4 font-semibold  transition duration-200`}
>
Return
</div>
          </div>
...
{rmbagDetails?.Material?.toUpperCase() === 'METAL' ?(
  ...
  <p className="text-black font-semibold font-sm">
  {rmbagDetails.wt}|{rmbagDetails.Pcs} (Ctw|Pcs)
    </p>)}
               <div className='flex flex-col gap-0 '>
